...
The Coding Agent GUI was rebuilt from a Vite + Astryx localhost web app into a Tauri 2 desktop application. The renderer is now React + Vite + Tailwind v4 + shadcn/ui (all Astryx components replaced: Sidebar, Command/Dialog palette, Tabs inspector, Select workspace picker, Alert/Badge/Collapsible, react-markdown rendering). The backend is unchanged in spirit: `server.mjs` became the Node sidecar, with a configurable CORS origin allowlist (Tauri webview origins + loopback + vite dev), OPTIONS preflight, and a sidecar mode that never serves static assets. The Rust shell spawns the sidecar and kills it on exit; `contextIsolation` stays on and the webview has no Node access. Release mode bundles a Bun-compiled single-file sidecar binary (98 MB) next to the app; the NSIS installer was produced end-to-end. The renderer stays Web-portable via a Vite build mode (`--mode tauri`) that sets `VITE_API_BASE`, and the same `node server.mjs` still serves the browser version (web mode). Environment setup during the migration: Rust toolchain installed (rustup stable), Smart App Control disabled on the host because it blocked freshly compiled Rust build scripts, and the official Bun CLI was used because the npm `bun` package binary was incompatible with the host Windows version.
